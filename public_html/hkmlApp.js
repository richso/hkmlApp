$j(document).ready(function() {
    ...
    if (! appHead) {
        ...
        if (/^redirect\.php$/.test(lastLocSeg) || /^viewthread\.php$/.test(lastLocSeg)) { 
            ...
            try {
                var thumbnails = $('.t_row > tbody > tr > td:nth-child(2) img:not([smilieid]):not([src^="images/d-xite"]):not([src^="images/common"]):not([src^="https://www.hkml.net/Discuz/images/common"]):not([src^="https://hkml.net/Discuz/images/common"]):not([src^="images/attachicons"]):not([src^="http://wpa.qq.com/pa?p="]):not([src^="http://web.icq.com/whitepages/online?icq="]):not([src^="http://edit.yahoo.com/config/send_webmesg?.target="]):not([src^="http://blog.roodo.com/onion_club/"]):not([src^="http://amos1.taobao.com/"])');
                var urls = [];
        ...
        if (form && !form.elements['msgto']) {
                ...
                form.addEventListener('submit', async function(event) {
                        event.preventDefault(); // 阻止預設立即提交

                        // 3. 觸發原有的 validate() 驗證機制
                        if (typeof window.validate === 'function') {
                                if (window.validate(form) === false) {
                                        console.log('原始驗證未通過，取消提交。');
                                        return; 
                                }
                        }
                        ...
                        formData.delete(fileKey); // 先清除，稍後補上處理過的新檔案
                        ...
                        for (let i = 0; i < files.length; i++) {
                                ...
                                if (file && file.type.startsWith('image/')) {
                                        try {
                                                ...
                                                console.log(`處理完成，新檔名: ${newName} (EXIF 角度: ${orientation})`);
                                        } catch (error) {
                                                console.error(`處理 ${file.name} 失敗，將使用原檔案上傳。`, error);
                                                formData.append(fileKey, file); // 失敗後備方案
                                        }
                                } else {
                                        if (file && file.size > 0) {
                        ...
                        // 5. 將最終處理好的 FormData 發送到表單原本的 action
                ...
                function getExifOrientation(file) {
                        return new Promise((resolve) => {
                                ...
                                reader.onload = function(e) {
                                        ...
                                        if (view.getUint16(0, false) !== 0xFFD8) return resolve(-1); // 不是 JPEG
                                ...
                                // 只讀取前 64KB 即可獲取 EXIF 資訊，節省記憶體
                                reader.readAsArrayBuffer(file.slice(0, 65536));
                        });
                }

                /**
                 * 核心圖片處理：結合實體旋轉、長邊縮放 (Max 1024px) 與 70% 壓縮
                 */
                        ...
                        return new Promise((resolve, reject) => {
                                ...
                                reader.onload = function(event) {
                                        ...
                                        img.onload = function() {
                                                ...
                                                // 判斷 EXIF 角度：如果是 5, 6, 7, 8，代表圖片寬高在視覺上是顛倒的
                                                ...
                                                // 依據 EXIF Orientation 計算 Canvas 物理旋轉與位移
                                                // 此步驟會把像素真正移位，導正圖片，並且輸出的 JPEG 將不再帶有錯誤的 EXIF 轉角
                                                switch (orientation) {
                                                        case 2: ctx.transform(-1, 0, 0, 1, width, 0); break;                  // 左右鏡像
                                                        case 3: ctx.transform(-1, 0, 0, -1, width, height); break;             // 旋轉 180 度
                                                        case 4: ctx.transform(1, 0, 0, -1, 0, height); break;                 // 上下顛倒
                                                        case 5: ctx.transform(0, 1, 1, 0, 0, 0); break;                       // 旋轉 90 度 + 左右鏡像
                                                        case 6: ctx.transform(0, 1, -1, 0, width, 0); break;                  // 順時針旋轉 90 度
                                                        case 7: ctx.transform(0, -1, -1, 0, width, height); break;             // 旋轉 270 度 + 左右鏡像
                                                        case 8: ctx.transform(0, -1, 1, 0, 0, height); break;                  // 逆時針旋轉 90 度
                                                        default: break;                                                        // 正常（不需旋轉）
                                                }

                                                // 由於畫布已經過旋轉位移，此處必須映射回原始圖片的縮放比例來繪製
                                                const scaleX = width / targetWidth;
                                                const scaleY = height / targetHeight;
                                                ctx.drawImage(img, 0, 0, srcWidth * scaleX, srcHeight * scaleY);

                                                // 輸出為全新、無 EXIF 轉角殘留的真實 JPEG 檔案
                                                canvas.toBlob((blob) => {
